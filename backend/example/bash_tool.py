...
class BackgroundShell:
    """Background shell data container 后代进程数据容器.

    Pure data class that only stores state and output 只存储进程状态和输出行，不负责IO操作.
    IO operations are managed externally by BackgroundShellManager io操作由BackgroundShellManager管理.
    """
    ...
    def add_output(self, line: str):
        """Add new output line.添加新输出行"""
        self.output_lines.append(line)

    def get_new_output(self, filter_pattern: str | None = None) -> list[str]:
        """Get new output since last check, optionally filtered by regex.获取自上次检查以来的新输出行，可选正则表达式过滤。"""
    ...
    def update_status(self, is_alive: bool, exit_code: int | None = None):
        """Update process status.更新进程状态"""
        # 如果进程不活跃，更新状态为已完成或失败，根据退出码判断
        if not is_alive:
    ...
    async def terminate(self):
        """Terminate the background process.终止后台进程"""
        # 如果进程返回码为是none，说明进程还在运行中，尝试终止
...
class BackgroundShellManager:
    ...
    async def start_monitor(cls, bash_id: str) -> None:
        ...
        # 启动监控任务，主要为了监控进程的输出以及进程的状态
        async def monitor():
            try:
                # 获取当前进程对象
                process = shell.process
                # Continuously read output until process ends 持续性读取进程输出直到进程结束
                # 当进程还在执行
