...
class Agent:
    ...
    # 初始化智能体实例，需要提供llm_client、system_prompt、tools、max_steps、workspace_dir、token_limit参数
    def __init__(
        self,
    ...
    def _estimate_tokens(self) -> int:
        ...
        return total_tokens
    # 回退方法：当tiktoken库不可用时，使用简单的字符数计算
...
5. Do not include "user" related content, only summarize the Agent's execution process"""
    ...
    # 主循环，用于执行代理任务
    # 接收参数：self，cancel_event，其中cancel_event用于在任意时间取消执行任务且是可选参数
    # 返回：response，字符串类型，包含代理任务的执行结果
    async def run(self, cancel_event: Optional[asyncio.Event] = None) -> str:
        """Execute agent loop until task is complete or max steps reached.

        # 意思是：在任意时间可以设置cancel_event，来取消执行任务，在下一个安全点停止
        Args:
        ...
        # 主循环，当任务完成或达到最大步骤数时，退出循环
        while step < self.max_steps:
            # Check for cancellation at start of each step，每次开始前检查是否取消执行任务
            # 如果检查到取消，调用_cleanup_incomplete_messages方法清理不完整的的消息记录
            ...
            # 检查并总结当前步骤的消息记录，防止上下文溢出
            await self._summarize_messages()

            # Step header with proper width calculation，格式化输出当前步骤的标题，宽度为58个字符，用于描述执行进度
            BOX_WIDTH = 58
            ...
            try:
                ...
                # Check if it's a retry exhausted error，检查是否是重试耗尽错误
                # 如果是重试耗尽错误，记录错误信息并返回错误消息
            ...
            # Accumulate API reported token usage，累计当前步骤的API报告的token使用量
            if response.usage:
                self.api_total_tokens = response.usage.total_tokens

            # Log LLM response，记录当前步骤的LLM响应
            ...
            # Print thinking if present 打印思考过程
            if response.thinking:
                print(f"\n{Colors.BOLD}{Colors.MAGENTA}🧠 Thinking:{Colors.RESET}")
                print(f"{Colors.DIM}{response.thinking}{Colors.RESET}")

            # Print assistant response 打印assistant响应
            if response.content:
